 ...
 */
async function createCodyOpenAIConfig(
  config: CodyConnectorConfig = {}
): Promise<OpenAIConnectorConfig> {
  ...
  const mergedConfig = {
    ...
    ...(process.env['SRC_ENDPOINT'] && {
      endpoint: process.env['SRC_ENDPOINT'],
    }),
  ...
  if (!mergedConfig.accessToken) {
    throw new Error(
      'Access token is required. Set accessToken in config or SRC_ACCESS_TOKEN environment variable.'
    );
  }
  ...
  return {
    ...
    logger: mergedConfig.logger, // Pass through the logger
    defaultHeaders: {
 ...
 * ```
 */
export async function createCodyConnector(
  config: CodyConnectorConfig = {}
): Promise<OpenAIConnector> {
